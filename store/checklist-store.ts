...
interface ChecklistActions {
  // Template actions
  addTemplate: (
    template: Omit<ChecklistTemplate, "id" | "createdAt" | "updatedAt" | "usageCount" | "rating" | "reviews">,
  ) => void
  ...
  // Project actions
  createProject: (name: string, description: string, templateId: string, options?: Partial<ChecklistProject>) => void
  ...
  addChecklistItem: (item: Omit<ChecklistItem, "id" | "createdAt" | "updatedAt" | "attachments" | "comments">) => void
  ...
  getProjectStats: (projectId: string) => { total: number; completed: number; percentage: number; overdue: number }
  getTeamStats: () => { member: string; tasksCompleted: number; timeSpent: number }[]
...
const defaultTemplates: ChecklistTemplate[] = [
  {
    ...
    description: "Complete workflow for building modern web applications with React, Node.js, and deployment",
    ...
    items: [
      ...
      },
      {
        ...
        description: "Set up development tools, IDE, version control, and project structure",
        completed: false,
        category: "Setup",
        priority: "high",
      ...
      },
      {
        ...
        description: "Design database schema, set up database, and create migrations",
        completed: false,
        category: "Backend",
        priority: "high",
      ...
      },
      {
        ...
        description: "Build RESTful APIs with authentication, validation, and error handling",
        completed: false,
        category: "Backend",
        priority: "high",
        tags: ["api", "backend", "authentication", "validation"],
        estimatedTime: 480,
        notes: "Implement JWT authentication, input validation, and comprehensive error handling",
        dependencies: [],
        status: "todo",
      },
      {
      ...
      },
      {
        ...
        estimatedTime: 240,
        notes: "Choose appropriate state management solution based on app complexity",
        dependencies: [],
        status: "todo",
      },
      {
        ...
        estimatedTime: 300,
        notes: "Include password reset, email verification, and social login options",
        dependencies: [],
        status: "todo",
      },
      {
      ...
      },
      {
        ...
        description: "Optimize bundle size, implement caching, lazy loading, and CDN",
        completed: false,
      ...
      },
      {
        ...
        description: "Implement security best practices, HTTPS, CSP, and vulnerability scanning",
        completed: false,
        category: "Security",
        priority: "high",
      ...
      },
      {
        ...
        description: "Create comprehensive documentation, API docs, and deployment guides",
        completed: false,
        category: "Documentation",
        priority: "medium",
        tags: ["documentation", "api-docs", "readme"],
        estimatedTime: 180,
        notes: "Include setup instructions, API reference, and troubleshooting guide",
        dependencies: [],
        status: "todo",
      },
      {
        title: "Deployment & DevOps",
        description: "Set up production deployment, monitoring, and CI/CD pipeline",
        completed: false,
        category: "DevOps",
        priority: "high",
        tags: ["deployment", "cicd", "monitoring", "devops"],
        estimatedTime: 300,
        notes: "Include staging environment, automated deployments, and monitoring setup",
        dependencies: [],
        status: "todo",
      },
    ],
    ...
    (set, get) => ({
      ...
      createProject: (name, description, templateId, options = {}) => {
        ...
        set((state) => ({
          ...
          templates: state.templates.map((t) => (t.id === templateId ? { ...t, usageCount: t.usageCount + 1 } : t)),
        }))
      },
      ...
      deleteChecklistItem: (id) =>
        set((state) => ({
          activeChecklists: state.activeChecklists.filter((item) => item.id !== id),
          projects: state.projects.map((project) => ({
            ...project,
      ...
      bulkUpdateItems: (ids, updates) =>
        set((state) => ({
          activeChecklists: state.activeChecklists.map((item) =>
            ids.includes(item.id) ? { ...item, ...updates, updatedAt: new Date() } : item,
          ),
          projects: state.projects.map((project) => ({
            ...project,
            items: project.items.map((item) =>
      ...
      addComment: (itemId, comment) => {
        ...
        set((state) => ({
          activeChecklists: state.activeChecklists.map((item) =>
            item.id === itemId ? { ...item, comments: [...item.comments, newComment], updatedAt: new Date() } : item,
          ),
          projects: state.projects.map((project) => ({
            ...project,
            items: project.items.map((item) =>
      ...
      assignItem: (id, assigneeId) =>
        set((state) => ({
          activeChecklists: state.activeChecklists.map((item) =>
            item.id === id ? { ...item, assignee: assigneeId, updatedAt: new Date() } : item,
          ),
          projects: state.projects.map((project) => ({
            ...project,
            items: project.items.map((item) =>
          ...
          projects: state.projects.map((project) => ({
            ...
            timeEntries: project.timeEntries.map((entry) => {
              if (entry.itemId === itemId && !entry.endTime) {
                const duration = Math.floor((now.getTime() - entry.startTime.getTime()) / 60000)
      ...
      addTimeEntry: (entry) =>
        set((state) => ({
          projects: state.projects.map((project) => ({
            ...project,
            timeEntries: [...project.timeEntries, { ...entry, id: crypto.randomUUID() }],
          })),
        })),
      ...
      updateTimeEntry: (id, updates) =>
        set((state) => ({
          projects: state.projects.map((project) => ({
            ...project,
            timeEntries: project.timeEntries.map((entry) => (entry.id === id ? { ...entry, ...updates } : entry)),
          })),
        })),
      ...
      getAnalytics: () => {
        ...
        const allItems = [...activeChecklists, ...projects.flatMap((p) => p.items)]
        ...
        const totalTimeSpent = projects.flatMap((p) => p.timeEntries).reduce((sum, entry) => sum + entry.duration, 0)
        ...
        const categoryBreakdown = allItems.reduce(
          (acc, item) => {
        ...
        const teamPerformance = get().teamMembers.map((member) => ({
          member: member.name,
          tasksCompleted: allItems.filter((item) => item.assignee === member.id && item.completed).length,
          timeSpent: projects
      ...
      getProjectStats: (projectId) => {
        ...
        if (!project) return { total: 0, completed: 0, percentage: 0, overdue: 0 }
